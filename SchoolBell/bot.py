...
async def send_random_fact_for_year(message: types.Message, year: int, show_year_prefix: bool = False):
...
async def send_random_fact_for_user_birth_year(message: types.Message):
    ...
    await send_random_fact_for_year(message, int(user["birth_year"]), show_year_prefix=False)
...
async def admin_add_fact_image(message: types.Message, state: FSMContext):
    ...
    await message.answer("Теперь отправь текст карточки.")
...
@dp.callback_query(AddFactStates.waiting_for_category, F.data.startswith("cat:"))
...
async def admin_edit_weight_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.update_data(fact_id=fact_id)
    await callback.message.answer("Выбери новый вес карточки:", reply_markup=admin_weight_kb())
    await callback.answer()


@dp.callback_query(EditStates.waiting_for_weight, F.data.startswith("weight:"))
async def admin_edit_weight_save(callback: types.CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer()
        return
    ...
    fact = db.get_fact_by_id(fact_id)
    await callback.message.answer("✅ Вес обновлён.", reply_markup=admin_menu())
    await send_fact_message(callback.message, fact, with_admin_controls=True)
    await callback.answer()
...
async def admin_edit_category_process(callback: types.CallbackQuery, state: FSMContext):
    ...
    fact = db.get_fact_by_id(fact_id)
    await callback.message.answer("✅ Категория обновлена.", reply_markup=admin_menu())
    await send_fact_message(callback.message, fact, with_admin_controls=True)
    await callback.answer()
...
async def handle_all(message: types.Message, state: FSMContext):
    ...
    if text == "🎲 Факт моего года":
        await send_random_fact_for_user_birth_year(message)
        return
